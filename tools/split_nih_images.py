...
def load_train_ids(pkl_path):
    """載入 train split (img_id.pkl)"""
    ids = []
    with open(pkl_path, "r") as f:
        for line in f:
            ids.append(line.strip())
    print(f"Loaded {len(ids)} train images from {pkl_path}")
    return ids
...
def move_images(ids, split_name, filename_to_path, output_root, mode="copy"):
    """把圖片搬到 split 資料夾"""
...
def main(args):
    ...
    train_ids = load_train_ids(args.train_pkl)
